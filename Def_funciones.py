...
def busqueda_malla(datos, lista_cfg, n_prueba, metodo = "sarima", paralelo = True):
    ...
    if paralelo:
        ...
        tareas = (delayed(calificar_modelo)(datos, n_prueba, metodo, cfg) for cfg
        in lista_cfg)
        resultados = ejecutor(tareas)
    else:
        resultados = [calificar_modelo(datos, n_prueba, metodo, cfg) for cfg in
        lista_cfg]
...
def calificar_modelo(datos, n_prueba, metodo, cfg, debug = False):
    ...
    llave = str(cfg)
    if debug:
        resultado, predicciones = validacion_al_frente(datos, n_prueba, metodo,
        cfg)
    else:
        ...
        # nunca mostrar advertencias cuando se busca en malla...demasiado "ruido"
            with catch_warnings():
                ...
                resultado, predicciones = validacion_al_frente(datos, n_prueba,
                metodo, cfg)
        except:
            error = None
